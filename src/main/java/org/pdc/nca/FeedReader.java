...
public class FeedReader {
	...
	public boolean loadConfiguration(String fileName)
	{
		...
		catch (org.apache.commons.configuration.ConfigurationException e)
		{
		  ...
		  logger.warn("Unable to load the configuration (reader.properties) file " + config.getFileName() + ", " + e.toString());
		  return false;
		}
	}
	...
	public List <Entry> getCandidateEntries (List <SyndEntry> entries)
	{
        ...
        for (SyndEntry entry : entries)
		{
			...
			if (cache.put(new Entry(entry)))
			{
				candidates.add(new Entry(entry));
				logger.info("Added to Cache " + entry.getTitle());
			}
			else
			{
				// do not log - a majority will be rejected. 
			}
        }
